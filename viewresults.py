...
st.cache()
def read_pandas():
    paths = list(pathlib.Path("gridresults").glob("*/*_report.json"))
    ...
    paths = list(pathlib.Path("gridresults").glob("*/CRFEntityExtractor_report.json")) 
    paths += list(pathlib.Path("gridresults").glob("*/DIETClassifier_report.json"))
